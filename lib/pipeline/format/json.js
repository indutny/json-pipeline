...
function JSON(sections, pipeline) {
  this.sections = sections;
  this.pipeline = pipeline;
}
module.exports = JSON;

JSON.prototype.parse = function parse(json) {
  ...
  if (this.sections.cfg) {
    ...
    for (var i = 0; i < blocks.length; i++) {
      var block = blocks[i];
      nodes[block.node] = pipeline.block(i === 0 ? 'start' : null);
...
JSON.prototype._parseNode = function _parseNode(json, index, nodes) {
  var node = json.nodes[index];
...
JSON.prototype._parseDominance = function _parseDominance(json, blocks) {
  var pipeline = this.pipeline;
...
JSON.prototype.render = function render() {
  var pipeline = this.pipeline;
...
JSON.prototype._renderCFG = function _renderCFG() {
  var pipeline = this.pipeline;
  var cfg = {
...
JSON.prototype._renderDominance = function _renderDominance() {
  var pipeline = this.pipeline;
  var dom = {
    blocks: new Array(pipeline.blocks.length)
  };
