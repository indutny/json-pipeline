...
Graphviz.prototype.escape = function escape(value) {
  return value.replace(/([^a-z0-9])/ig, '\\$1');
};
...
Graphviz.prototype._renderNode = function _renderNode(prefix, node) {
  var out = '';

  var label = '<S> i' + node.index + ' = ' + this.escape(node.opcode);
  if (node.literals.length > 0)
    label += this.escape('(' + node.literals.join(', ') + ')');
  ...
  out += prefix + 'N' + node.index + ' [\n';
  out += prefix + '  shape="record"\n';
  out += prefix + '  label="{{' + records.join('|') + '}}"\n';
  out += prefix + ']\n';
