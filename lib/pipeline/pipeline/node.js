...
Node.prototype._cutControl = function _cutControl() {
...
Node.prototype.replace = function replace(other) {
  ...
  for (var i = 0; i < this.control.length; i++) {
    var use = this.control[i];
    for (var j = 0; j < use.controlUses.length; j++) {
      var control = use.controlUses[j];
      if (control !== this)
        continue;

      use.controlUses[j] = other;
    }
  }
  ...
  other.controlUses = this.controlUses;
  this.controlUses = [];
};
