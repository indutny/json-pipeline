...
Node.prototype.removeInput = function removeInput(index) {
  var old = this.inputs[index];
  old._unuse(this, index);

  // Shift inputs to the left, and move their uses
...
Node.prototype.replace = function replace(other) {
  // Propagate location information through replacement
  if (other.loc === null)
    other.loc = this.loc;
  ...
  for (var i = 0; i < this.control.length; i++) {
    ...
    for (var j = 0; j < control.controlUses.length; j += 2) {
